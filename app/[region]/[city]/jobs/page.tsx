...
export default async function CityJobsPage({ params }: Props) {
    ...
    // Normalize city name for display (simple fix mostly, ideally we fetch "Zone Name" from DB or just capitalize)
    const formatCityName = (slug: string) => {
        return slug.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    };
    const cityName = formatCityName(city);

    // Fetch jobs passing the zone (city) filter
    // NOTE: 'city' from URL is a slug (e.g. 'rio-grande'). 'getJobs' expects 'zone'.
    // 'getJobs' has internal logic to normalize inputs, so passing the slug might work if zone storage is consistent.
    // However, usually we pass the search term. Let's pass the slug and rely on getJobs loose matching logic we verified earlier.
    const jobs = await getJobs({ region, zone: city });
        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            ...
            <div className="bg-white border-b sticky top-0 z-10">
                <div className="max-w-3xl mx-auto px-4 py-4">
                    <div className="mb-2">
                        <Link href={`/${region}/${city}`} className="text-sm text-gray-500 hover:text-gray-900 flex items-center gap-1">
                            <ArrowLeft size={16} />
                            Volver a locales de {cityName}
                        </Link>
                    </div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Oportunidades laborales en {cityName}, {regionName}
                    </p>
            ...
            <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
                {jobs.length === 0 ? (
                    <div className="text-center py-12">
                        ...
                        <h3 className="text-gray-900 font-medium">No hay empleos aquí todavía</h3>
                        <p className="text-gray-500 text-sm mt-1">
                            Se el primero en enterarte cuando publiquen.
                        </p>
                        <Link href={`/${region}/${city}`}>
                            <Button variant="outline" className="mt-4">
                                Volver a explorar locales
                            </Button>
                        </Link>
                    </div>
                ) : (
                                        ...
                                        <div className="flex flex-wrap gap-y-1 gap-x-3 mt-3 text-xs text-gray-500">
                                            <div className="flex items-center gap-1">
                                                ...
                                                {job.location_type === 'remote'
                                                    ? 'Remoto'
                                                    : (safeRender(job.venues?.address) || safeRender(job.venues?.zone) || cityName)}
                                            </div>
                                            <div className="flex items-center gap-1">
