...
export default function VenueDetailPage() {
    ...
    const [venueCoupons, setVenueCoupons] = useState<Coupon[]>([])
        ...
        try {
            ...
            if (venueData) {
                const mappedVenue: Venue = {
                    ...
                    ownerId: venueData.owner_id, // Add ownerId
                ...
                if (productsData) {
                    ...
                    setVenueProducts(mappedProducts)
                }
                ...
                if (couponsData) {
                    const mappedCoupons: Coupon[] = couponsData.map((c: any) => ({
                        id: c.id,
                        venueId: c.venue_id,
                        venueName: typeof venueData.name === 'object' ? venueData.name.es || venueData.name.en || "" : venueData.name,
                        code: c.code,
                        discount: c.discount,
                        type: c.type,
            ...
            const mockVenue = mockVenues.find((v) => v.slug === params.slug)
            if (mockVenue) {
                setVenue(mockVenue)
                setVenueCoupons(mockCoupons.filter((c) => c.venueId === mockVenue.id))
            } else {
                setError(true)
            }
        } finally {
    ...
    const visibleProducts = venueProducts.slice(0, features.productsLimit)
            ...
            <main className="container mx-auto px-4 py-6 max-w-4xl">
                ...
                <div className="grid md:grid-cols-[2fr_1fr] gap-8">
                    ...
                    <div className="space-y-8">
                        ...
                        {/* Active Coupons */}
                        {features.coupons && venueCoupons.length > 0 && (
                            <section>
